...
from EntFA.utils import (
    get_probability_parallel,
    prepare_cmlm_inputs,
    prepare_mlm_inputs,
    read_lines,
)
...
def build_models(args):
    prior_bart = BARTModel.from_pretrained(
        args.mlm_path, checkpoint_file="model.pt", data_name_or_path=args.mlm_path
    )
    prior_model = ConditionalSequenceGenerator(prior_bart)

    bart = BARTModel.from_pretrained(
        args.cmlm_model_path,
...
def extract_features(source, hypothesis, prior_model, model):
    ...
    for index in tqdm(range(len(hypothesis))):
        ...
        if len(ent_parts) > 0:
            ...
            pos_inputs = prepare_cmlm_inputs(
                source_doc, target_doc, ent_parts=ent_parts
            )

            # calculate probability features
            try:
                pri_probs = get_probability_parallel(
                    prior_model,
                    pri_inputs[0],
                    pri_inputs[1],
                    pri_inputs[2],
                ...
                pos_probs = get_probability_parallel(
                    model, pos_inputs[0], pos_inputs[1], pos_inputs[2], pos_inputs[3]
                )
                ...
                features.append(
                    (pos_inputs[3], pos_inputs[2], pri_probs, pos_probs, overlap)
                )
...
def infernece(test_features, classifier):
    ...
    x_mat = np.vstack(
        [x_mat[:, 0] / stds[0], x_mat[:, 1] / stds[1], x_mat[:, 2] / stds[2]]
    ).transpose()
