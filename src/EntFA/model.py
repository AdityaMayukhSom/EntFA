...
class ConditionalSequenceGenerator:
    ...
    def __init__(self, bart):
        ...
        if type(self.max_positions) is int:
    ...
    def tokenize_target(
        self, input_str: str | list[str], left_pad=False, append_eos=False
    ):
        ...
        for ins in input_str:
            ...
            if len(tokens.split(" ")) > min(self.max_positions) - calibration:
                tokens = " ".join(
                    tokens.split(" ")[: min(self.max_positions) - calibration]
                )
        ...
        prev_output_tokens = collate_tokens(
            prev_ids, pad_idx=1, left_pad=left_pad
        ).cuda()
    ...
    def tokenize_with_mask(self, input_str):
        ...
        Return:
            ...
            src_lengths (torch.Tensor): [batch_size]
        """
        input_ids = self.tokenizer(input_str, return_tensors="pt", padding=True)[
            "input_ids"
        ].cuda()
        input_lengths = torch.sum(input_ids != 1, dim=1).cuda()
        return input_ids, input_lengths
    ...
    def encode_decode(self, src_input, tgt_input, mask_filling=False):
        ...
        if mask_filling:
            ...
            prev_output_tokens, target, tgt_lengths = self.tokenize_target(
                tgt_input, left_pad=False, append_eos=True
            )
        else:
            src_tokens, src_lengths = self.tokenize(src_input, append_bos=False)
            prev_output_tokens, target, tgt_lengths = self.tokenize_target(
                tgt_input, left_pad=False
            )

        with torch.no_grad():
            encoder_out = self.bart.model.encoder(src_tokens, src_lengths=src_lengths)
            decoder_out = self.bart.model.decoder(
                prev_output_tokens, encoder_out=encoder_out, features_only=False
            )
            ...
            tgt_mask = (
                torch.arange(max_tgt_length)[None, :].cuda() < tgt_lengths[:, None]
            )
            tgt_token_probs.masked_fill_(not tgt_mask, 1.0)
    ...
    def generate(self, src_input, tgt_input=None):
        ...
        if tgt_input is not None:
            assert len(src_input) == len(tgt_input), (
                "source & target length should match."
            )
        ...
        with torch.no_grad():
            ...
            decoder_output = self.decode_sequence(
                encoder_output, target_ids=target_ids, prefix_tokens=[2]
            )
    ...
    def mask_filling(self, src_input, tgt_input=None):
        ...
        with torch.no_grad():
            encoder_output = self.encode_sequence(input_ids, lengths)
            decoder_output = self.decode_sequence(
                encoder_output, target_ids=target_ids, prefix_tokens=[2, 0]
            )
        return decoder_output
    ...
    def decode_sequence(
        ...
        for step in range(max_decode_step):
            decoder_outputs = self.bart.model.decoder(
                init_input, encoder_out, features_only=False
            )
            # logits: [batch_size, vocab]
            logits = decoder_outputs[0][:, -1, :]
            # attn: [batch_size, prev_token_len, src_token_len]
            attn = decoder_outputs[1]["attn"][0]

            if step + 1 < min_decode_step:
                logits[
                    :, eos_id
                ] = -math.inf  # mask <EOS> token when within minimal step
            logits[:, pad_id], logits[:, 0] = (
                -math.inf,
                -math.inf,
            )  # never select <PAD> & <BOS> token
            ...
            token_probs = (
                ...
                else torch.cat([token_probs, probs], dim=-1)
            )
            for t, s in zip(tokens, selected_token):
                t.append(
                    self.decode_func(s.unsqueeze(0)) if s.item() != pad_id else "<pad>"
                )
