...
def get_probability(position, tokens, probs, entity):
    ...
    assert position[1] in end_pos, "- {}\n- {}\n- {}\n- {}\n- {}\n".format(
        position, tokens, probs, entity, end_pos
    )
...
def get_probability_parallel(
    generator, src_input, tgt_input, position, entity, mask_filling=False
):
    ...
    """
    token_probs, target = generator.encode_decode(
        src_input, tgt_input=tgt_input, mask_filling=mask_filling
    )
